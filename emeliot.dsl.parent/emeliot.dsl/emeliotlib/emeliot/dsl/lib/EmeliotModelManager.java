...
public class EmeliotModelManager {
	private static final Logger LOG = Logger.getLogger(EmeliotModelManager.class);
	...
	public EPackage getEPackage(String packageName) {
		...
		return EmeliotResourceUtils.getEPackages(ecoreResources)
			.stream()
	...
	private void saveResources(String outputPath, Collection<Resource> resourceMap) throws IOException {
		for (var resource : resourceMap) {
			var fileName = EmeliotResourceUtils.getFileName(resource);
	...
	public Map<EObject, EObject> copyEcores(EmeliotModelManager otherModelManager) {
